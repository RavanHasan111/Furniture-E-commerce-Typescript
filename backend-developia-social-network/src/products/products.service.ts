...
export class ProductsService {
  ...
  findAll(sortParams: SortParams, filterParams?: FilterParams): Product[] {
    let filteredProducts = [...products]; // создаём копию, чтобы не мутировать оригинал
    ...
    if (filterParams) {
      ...
      if (filterParams.brand) {
        filteredProducts = filteredProducts.filter(
          product => product.brand === filterParams.brand
        );
      }

      if (filterParams.minRating !== undefined) {
        filteredProducts = filteredProducts.filter(
          product => product.rating >= filterParams.minRating!
        );
      }

      if (filterParams.maxRating !== undefined) {
        filteredProducts = filteredProducts.filter(
          product => product.rating <= filterParams.maxRating!
        );
      }
    }

    // Применяем сортировку с значениями по умолчанию
